...
import { RoofingContractor } from '../rfq-types';
import { MOCK_ROOFING_CONTRACTORS } from '../rfq-constants';
import { RFQStepBar } from './RFQStepBar';

interface RoofingContractorRFQProps {
  onBack: () => void;
...
const ContractorCard: React.FC<{ contractor: RoofingContractor, selected: boolean, onToggle: () => void }> = ({ contractor, selected, onToggle }) => (
  <div 
    onClick={onToggle}
...
export const RoofingContractorRFQ: React.FC<RoofingContractorRFQProps> = ({ onBack, onComplete }) => {
  const [step, setStep] = useState(1);
  const [state, setState] = useState<RoofingState>({
    requirementType: 'Complete New Roofing',
    propertyType: '',
    roofType: '',
    propertySize: '',
    floorsCount: '',
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Roofing <span className="text-luxury-gold">Requirement.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Choose the type of roofing work you need.
              </p>
            </div>

            <SectionWrapper title="Work Type" icon={Home} subtitle="Select requirement">
              <div className="grid grid-cols-1 gap-3">
                {[
                  { id: 'Complete New Roofing', desc: 'Full roofing system for a new structure.', icon: Sparkles, recommended: true },
                  { id: 'Replacement of Existing', desc: 'Removing old roof and installing new one.', icon: Trash2 },
                  { id: 'Roofing Extension', desc: 'Adding roof to an existing area.', icon: Maximize },
                  { id: 'Terrace Covering', desc: 'Covering an open terrace area.', icon: Layout },
                  { id: 'Leakage Rectification', desc: 'Fixing leaks and drainage issues.', icon: Droplets }
                ].map((item) => (
                  <button
                    key={item.id}
      ...
      case 2:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            ...
            <SectionWrapper title="Property Details" icon={Building2} subtitle="Type & Scale">
              <div className="space-y-6">
                <div className="space-y-3">
                  <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Property Type</p>
                  <SegmentedControl 
                    options={['House', 'Villa', 'Commercial', 'Industrial', 'Warehouse']} 
                    selected={state.propertyType} 
                    onChange={(val) => setState({ ...state, propertyType: val })} 
                  />
                </div>
                <div className="space-y-3">
                  <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Roof Type</p>
                  <SegmentedControl 
                    options={['Flat Roof', 'Sloped Roof', 'Curved Roof', 'Multi-Level']} 
                    selected={state.roofType} 
                    onChange={(val) => setState({ ...state, roofType: val })} 
                  />
                </div>
              </div>
            </SectionWrapper>
      ...
      case 3:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Roofing <span className="text-luxury-gold">System.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
            ...
            <SectionWrapper title="Systems" icon={Layers} subtitle="Select material">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Metal Sheets (GI/PPGL)', 'Polycarbonate Sheets', 'Clay Tiles (Mangalore)', 
                  'Concrete Tiles', 'Shingles', 'UPVC Sheets', 'FRP Sheets', 'Need Suggestion'
                ].map((sys) => (
                  <button
                    key={sys}
                    onClick={() => setState({ ...state, roofSystem: sys })}
                    className={`p-4 rounded-2xl text-[11px] font-bold border transition-all text-center ${
                      state.roofSystem === sys
            ...
            {state.roofSystem === 'Need Suggestion' && (
              <SectionWrapper title="Goal" icon={Sparkles} subtitle="What is your priority?">
                <div className="grid grid-cols-2 gap-2">
                  {[
                    { id: 'heat', label: 'Heat Reduction', icon: Sun },
                    { id: 'light', label: 'Natural Light', icon: Sparkles },
                    { id: 'budget', label: 'Budget Friendly', icon: ArrowUpRight },
                    { id: 'aesthetic', label: 'Premium Look', icon: Star },
                    { id: 'durability', label: 'Long Life', icon: Clock },
                    { id: 'noise', label: 'Noise Reduction', icon: Wind }
                  ].map((intent) => (
                    <button
                      key={intent.id}
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the quality range for your {state.roofSystem}.
              </p>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {[
                { name: 'Tata BlueScope', tags: ['Premium', 'Durable'] },
                { name: 'JSW Steel', tags: ['Reliable', 'Popular'] },
                { name: 'Everest', tags: ['High Performance'] },
                { name: 'Lexan', tags: ['Polycarbonate Expert'] }
              ].map((brand) => (
                <button
                  key={brand.name}
                  onClick={() => {
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            ...
            <SectionWrapper title="Scope" icon={ClipboardCheck} subtitle="What's included?">
              <div className="grid grid-cols-1 gap-2">
                {[
                  { id: 'structure', title: 'Support Structure (MS/Steel)', desc: 'Trusses, purlins, and frame.' },
                  { id: 'sheeting', title: 'Main Sheeting / Tiling', desc: 'Installation of primary roof material.' },
                  { id: 'insulation', title: 'Thermal Insulation', desc: 'Heat reduction layer (XLPE/Bubble).' },
                  { id: 'drainage', title: 'Gutter & Drainage System', desc: 'Rainwater handling and downpipes.' },
                  { id: 'flashing', title: 'Flashing & Ridge Capping', desc: 'Sealing joints and edges.' },
                  { id: 'skylight', title: 'Skylight / Ventilation', desc: 'Natural light and air flow.' }
                ].map((item) => (
                  <button
                    key={item.id}
                    onClick={() => {
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            ...
            <SectionWrapper title="Performance" icon={ShieldCheck} subtitle="Priorities">
              <div className="grid grid-cols-2 gap-2">
                {[
                  { id: 'leak-proof', label: '100% Leak Proof', icon: Droplets },
                  { id: 'heat', label: 'Heat Insulation', icon: Sun },
                  { id: 'wind', label: 'Wind Resistance', icon: Wind },
                  { id: 'noise', label: 'Noise Dampening', icon: Wind },
                  { id: 'drainage', label: 'Quick Drainage', icon: CloudRain },
                  { id: 'aesthetic', label: 'Modern Look', icon: Palette }
                ].map((item) => (
                  <button
                    key={item.id}
                    onClick={() => {
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            ...
            <div className="grid grid-cols-1 gap-3">
              {[
                ...
                { id: 'Complete Supply & Installation', desc: 'End-to-end service including structure and sheeting.', icon: Sparkles, recommended: true }
              ].map((item) => (
                <button
                  key={item.id}
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            ...
            <SectionWrapper title="Conditions" icon={Construction} subtitle="Site status">
              <div className="grid grid-cols-1 gap-2">
                {['Vacant Site', 'Under Construction', 'Occupied', 'High-Rise Access', 'Restricted Timings'].map(cond => (
                  <button
                    key={cond}
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Choose your roofing expert.
              </p>
            </div>

            <div className="space-y-4">
              {MOCK_ROOFING_CONTRACTORS.map((contractor) => (
                <ContractorCard 
                  key={contractor.id}
      ...
      <div className="px-2 pt-4">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="font-black text-[var(--ink)] tracking-tight">Roofing RFQ</span>
          </div>
          <div className="w-10" />
        </div>
