...
interface GrantsApprovalsRFQFlowProps {
  onBack: () => void;
  onComplete: () => void;
}

interface GrantsApprovalsState {
  // Step 1: Project Category & Context
  projectCategory: string;
  projectStage: string;
  authority: string;
  // Step 2: Grants / Approvals Required
  approvalsRequired: string[];
  // Step 3: Document Readiness
  documents: string[];
  applicationStatus: string;
  knownObjections: string;
  // Step 4: Service Expectation
...
export const GrantsApprovalsRFQFlow: React.FC<GrantsApprovalsRFQFlowProps> = ({ onBack, onComplete }) => {
  const [step, setStep] = useState(1);
  const [state, setState] = useState<GrantsApprovalsState>({
    projectCategory: '',
    projectStage: '',
    authority: '',
    approvalsRequired: [],
  ...
  const isStepValid = () => {
    if (step === 1) return state.projectCategory !== '' && state.projectStage !== '' && state.authority !== '';
  ...
  const generateRFQNumber = () => {
    ...
    return `TI360-GRNT-${random}`;
  };

  const handleFinalSubmit = () => {
    const rfqNum = generateRFQNumber();
    setState(prev => ({ ...prev, rfqNumber: rfqNum }));
    submitServiceRFQ('Grants Approvals', { ...state, rfqNumber: rfqNum });
    setStep(8);
  };
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the category and jurisdiction for your grants & approvals request.
              </p>
            </div>

            <SectionWrapper title="Project Context" icon={Building2} subtitle="Category & Stage">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Project Category</p>
                <SegmentedControl 
                  options={['Land Development / Layout', 'Residential Building', 'Commercial / Industrial', 'Institutional / Public Semi-Public', 'Change of Land Use (CLU)', 'Special Development Zone']} 
                  selected={state.projectCategory} 
                  onChange={(val) => setState({ ...state, projectCategory: val })} 
                />
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1 mt-4">Project Stage</p>
                <SegmentedControl 
                  options={['Pre-planning', 'Pre-application', 'Drawings Ready', 'Submission Ready', 'Under Review', 'Post-Sanction', 'Under Construction', 'Pre-Occupancy']} 
            ...
            <SectionWrapper title="Governing Authority" icon={Landmark} subtitle="Jurisdiction">
              <div className="grid grid-cols-1 gap-3">
                {[
                  'BMRDA / BDA / BIAPPA / DTCP', 
                  'BBMP (Bruhat Bengaluru Mahanagara Palike)', 
                  'KIADB / KSSIDC (Industrial)', 
                  'Panchayat / Local Planning Authority', 
                  'Not Sure / Need Identification'
                ].map((v) => (
                  <button
                    key={v}
      ...
      case 2:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Grants & <span className="text-luxury-gold">Approvals.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Select the specific statutory permissions required.
              </p>
            </div>
            ...
            <SectionWrapper title="Required Approvals" icon={ClipboardCheck} subtitle="Multi-select requirements">
              <div className="grid grid-cols-1 gap-2">
                {[
                  'Change of Land Use (CLU) / Conversion', 
                  'Layout Approval (LP) / LP Sanction', 
                  'Building Plan Sanction', 
                  'Commencement Certificate (CC)', 
                  'Occupancy Certificate (OC)', 
                  'Utility Connection Grants (BWSSB/BESCOM)', 
                  'Environmental Clearance (EC)', 
                  'Fire Force Clearance / NOC', 
                  'Height Clearance (AAI/HAL)', 
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define your current documentation and compliance status.
              </p>
            </div>
            ...
            <SectionWrapper title="Available Documents" icon={FileText} subtitle="Multi-select availability">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Registered Sale Deed / Title Deeds', 
                  'RTC (Pahani) / Mutation Records', 
                  'Village Map / Survey Sketch', 
                  'Conversion Order (ALU)', 
                  'Zonal Regulations Certificate', 
                  'Architectural Conceptual Layout', 
                  'Boundary Survey Report', 
                  'Existing NOCs / Approvals'
                ].map((doc) => (
                  <button
                    key={doc}
                    onClick={() => {
            ...
            <SectionWrapper title="Application Context" icon={History} subtitle="Status & Objections">
              <div className="space-y-6">
                <div className="space-y-3">
                  ...
                  <SegmentedControl 
                    options={['Fresh Application', 'Resubmission', 'Objection Case', 'Pending Follow-up', 'Correction / Revision Case', 'Not Sure']} 
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the depth of support required for your approvals.
              </p>
            </div>
            ...
            <SectionWrapper title="Support Scope" icon={Zap} subtitle="Multi-select expectations">
              <div className="grid grid-cols-1 gap-2">
                {[
                  ...
                  'Utility Approval Support', 
                  'Inspection Coordination', 
                  'Occupancy-stage Coordination', 
            ...
            <SectionWrapper title="Support Depth" icon={ShieldAlert} subtitle="Management Level">
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  'Authority Liaisoning & Follow-up', 
                  'Objection Scrutiny & Resolution', 
                  'Technical Presentation to Authority', 
      ...
      case 5:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            ...
            <SectionWrapper title="Timeline & Tracking" icon={Clock} subtitle="Urgency & Updates">
              <div className="space-y-6">
                <div className="space-y-3">
                ...
                <div className="space-y-3">
                  <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Tracking Expectations</p>
                  <div className="grid grid-cols-2 gap-2">
                    {[
                      'Status Tracking', 
                      'Milestone Reporting', 
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[var(--muted)] text-xs font-medium leading-relaxed">
                      Our system will analyze your project category, authority jurisdiction, and approval complexity to route your RFQ to the most qualified approval specialist in our network.
                    </p>
                  </div>
        ...
        return (
          ...
          >
            <div className="text-center space-y-2">
              ...
              <p className="text-[var(--muted)] text-xs font-semibold">Verify your grants & approvals requirements.</p>
            </div>
            ...
            <div className="space-y-6">
              <ReviewSection 
                ...
                icon={Building2} 
                items={[
                  { label: 'Category', value: state.projectCategory },
        ...
        return (
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <p className="text-[var(--muted)] font-medium">Your grants & approvals request is now live.</p>
            </div>
