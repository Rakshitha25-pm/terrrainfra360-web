...
interface LegalOpinionRFQFlowProps {
  onBack: () => void;
  onComplete: () => void;
}
...
interface LegalState {
  ...
  // Step 3: Expectations & Risk
  opinionFormat: string;
  riskSensitivity: string;
  concernAreas: string[];
  futureRiskAssessment: string;
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-4 mb-4">
      ...
      <div className="space-y-1 text-right">
        <p className="text-[9px] font-bold text-[var(--muted)] uppercase tracking-widest">Opinions Done</p>
...
export const LegalOpinionRFQFlow: React.FC<LegalOpinionRFQFlowProps> = ({ onBack, onComplete }) => {
  ...
  const isStepValid = () => {
    ...
    if (step === 3) return state.opinionFormat !== '' && state.riskSensitivity !== '' && state.concernAreas.length > 0 && state.futureRiskAssessment !== '';
    if (step === 4) return state.timeline !== '' && state.priorityLevel !== '' && state.consultationRequired !== '';
    if (step === 5) {
  ...
  const generateRFQNumber = () => {
    ...
    return `TI360-LEG-${random}`;
  };

  const handleFinalSubmit = () => {
    const rfqNum = generateRFQNumber();
    setState(prev => ({ ...prev, rfqNumber: rfqNum }));
    submitServiceRFQ('Legal Opinion', { ...state, rfqNumber: rfqNum });
    setStep(7);
  };
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the legal intent and property jurisdiction.
              </p>
            </div>

            <SectionWrapper title="Primary Purpose" icon={Gavel} subtitle="Decision support intent">
            ...
            <SectionWrapper title="Property Classification" icon={Building2} subtitle="Type & Jurisdiction">
              <div className="space-y-4">
                ...
                <SegmentedControl 
                  options={['Residential', 'Commercial', 'Land/Plot', 'Mixed Use']} 
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Property status and available legal chain.
              </p>
            </div>

            <SectionWrapper title="Property Stage" icon={Clock} subtitle="Current status">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Planning to Buy', 'Already Purchased', 
                  'Under Construction', 'Facing Legal Issue'
                ].map((stage) => (
                  <button
                    key={stage}
            ...
            <SectionWrapper title="Document Availability" icon={ClipboardCheck} subtitle="Multi-select available files">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Sale Deed', 'Previous Title Chain', 'Encumbrance (EC)', 
                  'Khata Cert/Extract', 'Approved Plan', 'Tax Receipts', 
                  'Conversion Cert', 'Occupancy Cert', 'No Documents'
                ].map((doc) => (
                  <button
                    key={doc}
                    onClick={() => {
      ...
      case 3:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Expectations & <span className="text-luxury-gold">Risk.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the depth and focus of the legal review.
              </p>
            </div>

            <SectionWrapper title="Opinion Format" icon={FileText} subtitle="Deliverable expectation">
              <div className="grid grid-cols-1 gap-3">
                {[
                  'Verbal Opinion', 
                  ...
                  <button
                    key={format}
                    onClick={() => setState({ ...state, opinionFormat: format })}
                    className={`p-4 rounded-2xl text-[13px] font-black border transition-all text-left ${
                      state.opinionFormat === format
                        ? 'bg-luxury-gold/5 border-luxury-gold text-luxury-gold shadow-lg shadow-luxury-gold/5'
                        : 'bg-[var(--paper)] border-[var(--line)] text-[var(--ink)] hover:border-[var(--line)]'
                    }`}
                  >
            ...
            <SectionWrapper title="Risk & Concerns" icon={ShieldAlert} subtitle="Sensitivity & focus">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Risk Sensitivity Level</p>
                <SegmentedControl 
                  options={['Basic Check', 'Moderate Review', 'Deep Due Diligence']} 
                  selected={state.riskSensitivity} 
                  onChange={(val) => setState({ ...state, riskSensitivity: val })} 
                />
                
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1 mt-6">Specific Concern Areas (Multi-select)</p>
                <div className="grid grid-cols-2 gap-2">
                  {[
                    'Title Ownership', 'Encumbrance Issues', 'Approval Validity', 
                    'Khata Legitimacy', 'Land Conversion', 'Builder Credibility', 
                ...
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1 mt-6">Future Risk Assessment Included?</p>
                <SegmentedControl 
                  options={['Yes', 'No']} 
                  selected={state.futureRiskAssessment} 
                  onChange={(val) => setState({ ...state, futureRiskAssessment: val })} 
                />
              </div>
            </SectionWrapper>
          </motion.div>
      ...
      case 4:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Timeline & <span className="text-luxury-gold">Consultation.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Urgency and expert interaction preference.
              </p>
            </div>

            <SectionWrapper title="Timeline" icon={Calendar} subtitle="Delivery schedule">
              <div className="space-y-4">
            ...
            <SectionWrapper title="Consultation" icon={Users} subtitle="Expert interaction">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Consultation Required (Call/Meeting)?</p>
                <SegmentedControl 
                  options={['Yes', 'No']} 
            ...
            <SectionWrapper title="Specific Concerns" icon={FileText} subtitle="Optional background">
              <textarea 
                value={state.specificConcern}
                onChange={(e) => setState({ ...state, specificConcern: e.target.value })}
                placeholder="Any specific concern or background you want us to know..."
              ...
              {state.selectionMode === 'manual' && (
                ...
                >
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-[11px] font-black text-[var(--ink)] uppercase tracking-wider">Available Legal Experts</h3>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[var(--muted)] text-xs font-medium leading-relaxed">
                      Our system will analyze your property type, jurisdiction, and concerns to route your RFQ to the most qualified legal expert in our network.
                    </p>
                  </div>
          ...
          >
            <div className="text-center space-y-2">
              <div className="w-20 h-20 bg-luxury-gold/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <Gavel className="w-10 h-10 text-luxury-gold" />
              </div>
              <h2 className="text-2xl font-black text-[var(--ink)]">Review & Submit</h2>
              <p className="text-[var(--muted)] text-xs font-semibold">Verify your legal opinion requirements.</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <ReviewSection 
                ...
                icon={ShieldAlert} 
                items={[
                  { label: 'Format', value: state.opinionFormat },
                  { label: 'Risk Level', value: state.riskSensitivity },
          ...
          >
            ...
            <div className="space-y-3">
              <h2 className="text-3xl font-black text-[var(--ink)] leading-tight">
                Legal RFQ <span className="text-luxury-gold">Submitted!</span>
              </h2>
              <p className="text-[var(--muted)] text-sm font-semibold max-w-xs mx-auto">
                Thank you, {state.userName}. Your legal opinion request has been sent to the selected experts.
              </p>
            </div>
    ...
    <div className="fixed inset-0 bg-premium-cream z-[60] flex flex-col overflow-hidden">
      ...
      <div className="bg-[var(--paper)]/80 backdrop-blur-xl border-b border-[var(--line)] px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex flex-col items-center">
          <h1 className="text-sm font-bold text-[var(--ink)] tracking-tight">Legal Opinion RFQ</h1>
