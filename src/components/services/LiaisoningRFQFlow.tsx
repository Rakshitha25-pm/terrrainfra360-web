...
interface LiaisoningRFQFlowProps {
  onBack: () => void;
  onComplete: () => void;
}

interface LiaisoningState {
  // Step 1: Project & Authority Context
  projectType: string;
  projectStage: string;
  authority: string;
  // Step 2: Liaisoning Scope
  scope: string[];
...
export const LiaisoningRFQFlow: React.FC<LiaisoningRFQFlowProps> = ({ onBack, onComplete }) => {
  const [step, setStep] = useState(1);
  const [state, setState] = useState<LiaisoningState>({
    projectType: '',
    projectStage: '',
    authority: '',
    scope: [],
  ...
  const isStepValid = () => {
    ...
    if (step === 2) return state.scope.length > 0;
    if (step === 3) return state.documents.length > 0 && state.caseType !== '';
    if (step === 4) return state.readiness.length > 0;
    if (step === 5) return state.outcomes.length > 0;
    if (step === 6) return state.complexity !== '' && state.urgency !== '' && state.supportLevel !== '' && state.trackingExpectation !== '';
    if (step === 7) {
  ...
  const generateRFQNumber = () => {
    ...
    return `TI360-LIA-${random}`;
  };

  const handleFinalSubmit = () => {
    const rfqNum = generateRFQNumber();
    setState(prev => ({ ...prev, rfqNumber: rfqNum }));
    submitServiceRFQ('Liaisoning', { ...state, rfqNumber: rfqNum });
    setStep(9);
  };
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the context and jurisdiction for your liaisoning request.
              </p>
            </div>

            <SectionWrapper title="Project Details" icon={Building2} subtitle="Type & Stage">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Project Type</p>
                <SegmentedControl 
                  options={['Residential', 'Commercial', 'Mixed Use', 'Apartment', 'Villa', 'Layout', 'Institutional']} 
                ...
                <SegmentedControl 
                  options={['Pre-planning', 'Pre-sanction', 'Post-sanction', 'Under Construction', 'Pre-occupancy', 'Utility Handover']} 
            ...
            <SectionWrapper title="Governing Authority" icon={Gavel} subtitle="Jurisdiction">
      ...
      case 2:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Liaisoning <span className="text-luxury-gold">Scope.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Select the specific coordination tasks required.
              </p>
            </div>

            <SectionWrapper title="Service Scope" icon={ClipboardCheck} subtitle="Multi-select requirements">
              <div className="grid grid-cols-1 gap-2">
                {[
                  'Building Plan Approval Coordination', 
                  'Commencement Certificate (CC) Follow-up', 
                  'Occupancy Certificate (OC) Coordination', 
                  'Khata Transfer / Bifurcation Coordination', 
                  'BWSSB NOC & Connection Coordination', 
                  'BESCOM Power Sanction & Connection', 
                  'Fire Force Clearance / NOC', 
                  'KSPCB Consent (Pollution Control)', 
                  'Road Cutting / Trenching Permission', 
                  'End-to-End Authority Liaisoning'
                ].map((item) => (
                  <button
                    key={item}
                    onClick={() => {
                  ...
                  >
                    <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${state.scope.includes(item) ? 'bg-luxury-gold border-luxury-gold text-white' : 'bg-[var(--paper)] border-[var(--line)]'}`}>
                      {state.scope.includes(item) && <Check size={12} strokeWidth={4} />}
                    </div>
                    {item}
                  </button>
                ))}
      ...
      case 3:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Documentation <span className="text-luxury-gold">Status.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the current availability of documents and case type.
              </p>
            </div>
            ...
            <SectionWrapper title="Available Documents" icon={FileText} subtitle="Multi-select availability">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  'Survey Sketch / Tippani / Podi', 
                  'Architectural Submission Drawings', 
                  'Structural Stability Certificate', 
                  'Soil Test Report', 
                  'Prior Sanctioned Plan (if any)', 
            ...
            <SectionWrapper title="Case Type" icon={History} subtitle="Application context">
              <div className="grid grid-cols-1 gap-3">
                {[
                  'Fresh Application', 
                  'Resubmission', 
                  'Correction Case', 
                  'Deviation Regularization Follow-up', 
                  'Pending File Follow-up'
                ].map((type) => (
                  <button
                    key={type}
                    onClick={() => setState({ ...state, caseType: type })}
                    className={`p-4 rounded-2xl text-[13px] font-black border transition-all text-left ${
                      state.caseType === type
                        ? 'bg-luxury-gold/5 border-luxury-gold text-luxury-gold shadow-lg shadow-luxury-gold/5'
                        : 'bg-[var(--paper)] border-[var(--line)] text-[var(--ink)] hover:border-[var(--line)]'
      ...
      case 4:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Readiness <span className="text-luxury-gold">Status.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the technical readiness and prior history.
              </p>
            </div>

            <SectionWrapper title="Project Readiness" icon={CheckSquare} subtitle="Technical status">
              <div className="grid grid-cols-1 gap-2">
                {[
                  'Site Measurements & Boundaries Finalized', 
                  'Architectural Drawings Ready for Submission', 
                  'Structural Design & Details Available',
                  'Basic NOCs (Fire/Airport) Already Obtained',
                  'No Prior Objections or Notices Received', 
                  'Pending Objections Need Resolution'
                ].map((item) => (
                  <button
                    key={item}
                    onClick={() => {
                      const newReadiness = state.readiness.includes(item)
                        ? state.readiness.filter(r => r !== item)
                        : [...state.readiness, item];
                      setState({ ...state, readiness: newReadiness });
                    }}
                    className={`p-4 rounded-2xl text-[12px] font-bold border transition-all flex items-center gap-3 ${
                      state.readiness.includes(item)
                        ? 'bg-luxury-gold/10 border-luxury-gold text-luxury-gold shadow-lg shadow-luxury-gold/5'
                        : 'bg-[var(--bg)] border-[var(--line)] text-[var(--muted)] hover:bg-[var(--paper)]'
                    }`}
                  >
                    <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${state.readiness.includes(item) ? 'bg-luxury-gold border-luxury-gold text-white' : 'bg-[var(--paper)] border-[var(--line)]'}`}>
                      {state.readiness.includes(item) && <Check size={12} strokeWidth={4} />}
                    </div>
                    {item}
                  </button>
                ))}
      ...
      case 5:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Outcome <span className="text-luxury-gold">Expectations.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the specific deliverables and coordination depth.
              </p>
            </div>

            <SectionWrapper title="Expected Outcomes" icon={Zap} subtitle="Multi-select deliverables">
              <div className="grid grid-cols-1 gap-2">
                {[
                  'Detailed Document Scrutiny & Gap Analysis', 
                  'Authority Filing & Online Submission', 
                  'Physical File Follow-up & Liaisoning', 
                  'Objection Resolution & Resubmission', 
                  'Departmental Inspection Coordination', 
                  'Demand Note Generation & Fee Coordination', 
                  'Final Approval / Certificate Collection'
                ].map((item) => (
                  <button
                    key={item}
                    onClick={() => {
                      const newOutcomes = state.outcomes.includes(item)
                        ? state.outcomes.filter(o => o !== item)
                        : [...state.outcomes, item];
                      setState({ ...state, outcomes: newOutcomes });
                    }}
                    className={`p-4 rounded-2xl text-[12px] font-bold border transition-all flex items-center gap-3 ${
                      state.outcomes.includes(item)
                        ? 'bg-luxury-gold/10 border-luxury-gold text-luxury-gold shadow-lg shadow-luxury-gold/5'
                        : 'bg-[var(--bg)] border-[var(--line)] text-[var(--muted)] hover:bg-[var(--paper)]'
                    }`}
                  >
                    <div className={`w-5 h-5 rounded-md border flex items-center justify-center transition-colors ${state.outcomes.includes(item) ? 'bg-luxury-gold border-luxury-gold text-white' : 'bg-[var(--paper)] border-[var(--line)]'}`}>
                      {state.outcomes.includes(item) && <Check size={12} strokeWidth={4} />}
                    </div>
                    {item}
                  </button>
                ))}
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the difficulty level and delivery timeline.
              </p>
            </div>
            ...
            <SectionWrapper title="Urgency & Support" icon={Clock} subtitle="Timeline & Depth">
              <div className="space-y-4">
                ...
                />
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1 mt-4">Support Level</p>
                <SegmentedControl 
                  options={['Submission Only', 'Follow-up Only', 'Full Ownership']} 
                  selected={state.supportLevel} 
                  onChange={(val) => setState({ ...state, supportLevel: val })} 
                />
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1 mt-4">Tracking Expectation</p>
                <SegmentedControl 
                  options={['Status Tracking', 'Milestone Updates', 'Escalation Support']} 
                  selected={state.trackingExpectation} 
                  onChange={(val) => setState({ ...state, trackingExpectation: val })} 
                />
              </div>
            </SectionWrapper>
          </motion.div>
      ...
      case 7:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Specialist <span className="text-luxury-gold">Selection.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Choose your liaisoning expert or let us suggest the best fit.
              </p>
            </div>
              ...
              {state.selectionMode === 'manual' && (
                ...
                >
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-[11px] font-black text-[var(--ink)] uppercase tracking-wider">Liaisoning Specialists</h3>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[var(--muted)] text-xs font-medium leading-relaxed">
                      Our system will analyze your project context, authority jurisdiction, and coordination complexity to route your RFQ to the most qualified liaisoning specialist in our network.
                    </p>
                  </div>
        ...
        return (
          ...
          >
            <div className="text-center space-y-2">
              ...
              <p className="text-[var(--muted)] text-xs font-semibold">Verify your liaisoning requirements.</p>
            </div>
            ...
            <div className="space-y-6">
              ...
              <ReviewSection 
                ...
                items={[
                  ...
                  { label: 'Case Type', value: state.caseType }
                ]}
              />
              ...
              <ReviewSection 
                ...
                icon={Clock} 
                items={[
                  { label: 'Complexity', value: state.complexity },
                  { label: 'Urgency', value: state.urgency },
                  { label: 'Support', value: state.supportLevel }
                ]}
              />
        ...
        return (
          ...
          >
            ...
            <div className="space-y-3">
              ...
              <p className="text-[var(--muted)] font-medium">Your liaisoning request is now live.</p>
            </div>
        ...
        <div className="p-6 bg-[var(--paper)] border-t border-[var(--line)]">
          <div className="max-w-4xl mx-auto">
            <button
              ...
              onClick={() => step === 8 ? handleFinalSubmit() : setStep(step + 1)}
            ...
            >
              {step === 8 ? 'Publish RFQ' : 'Next Section'}
              <ArrowRight size={18} />
            </button>
          </div>
        </div>
