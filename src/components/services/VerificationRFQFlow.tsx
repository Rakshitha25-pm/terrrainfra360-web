...
interface VerificationRFQFlowProps {
  onBack: () => void;
  onComplete: () => void;
}
...
interface VerificationState {
  ...
  // Step 2: Property & Stage
  propertyType: string;
  jurisdiction: string;
  transactionStage: string;
  // Step 3: Documents & Depth
  documentsAvailable: string[];
  verificationDepth: string;
  // Step 4: Concerns & Site Visit
  ...
  >
    ...
    <div className="grid grid-cols-2 gap-4 mb-4">
      ...
      <div className="space-y-1 text-right">
        <p className="text-[9px] font-bold text-[var(--muted)] uppercase tracking-widest">Verifications</p>
...
export const VerificationRFQFlow: React.FC<VerificationRFQFlowProps> = ({ onBack, onComplete }) => {
  ...
  const isStepValid = () => {
    if (step === 1) return state.purpose !== '' && state.verificationScope.length > 0;
    if (step === 2) return state.propertyType !== '' && state.jurisdiction !== '' && state.transactionStage !== '';
    if (step === 3) return state.documentsAvailable.length > 0 && state.verificationDepth !== '';
    if (step === 4) return state.specificConcerns.length > 0 && state.siteVerificationRequired !== '';
    if (step === 5) return state.outputExpectation !== '' && state.consultationRequired !== '' && state.timeline !== '' && state.priorityLevel !== '';
    if (step === 6) {
  ...
  const generateRFQNumber = () => {
    ...
    return `TI360-VER-${random}`;
  };

  const handleFinalSubmit = () => {
    const rfqNum = generateRFQNumber();
    setState(prev => ({ ...prev, rfqNumber: rfqNum }));
    submitServiceRFQ('Verification', { ...state, rfqNumber: rfqNum });
    setStep(8);
  };
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Purpose & <span className="text-luxury-gold">Scope.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the intent and items to be verified.
              </p>
            </div>

            <SectionWrapper title="Verification Purpose" icon={Fingerprint} subtitle="Primary intent">
              <div className="grid grid-cols-1 gap-3">
                {[
                  'Property Purchase Verification', 
                  'Owner Authenticity Check', 
                  'Builder Background Check', 
                  'Document Authenticity Validation', 
                  'Pre-Registration Verification', 
                  'Loan/Bank Requirement', 
                  'General Risk Check'
                ].map((p) => (
                  <button
                    key={p}
            ...
            <SectionWrapper title="Verification Scope" icon={Eye} subtitle="Multi-select items">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Property Documents', 'Owner Identity', 'Title Ownership', 
                  'Builder Credentials', 'Approvals & Permissions', 'Land Records', 
                  'All of the Above'
                ].map((item) => (
                  <button
                    key={item}
                    onClick={() => {
                      const newScope = state.verificationScope.includes(item)
                        ? state.verificationScope.filter(s => s !== item)
                        : [...state.verificationScope, item];
                      setState({ ...state, verificationScope: newScope });
                    }}
                    className={`p-4 rounded-2xl text-[11px] font-bold border transition-all relative flex items-center justify-center text-center ${
                      state.verificationScope.includes(item)
      ...
      case 2:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Property & <span className="text-luxury-gold">Stage.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define the context and transaction progress.
              </p>
            </div>

            <SectionWrapper title="Property Context" icon={Building2} subtitle="Type & Jurisdiction">
              <div className="space-y-4">
            ...
            <SectionWrapper title="Transaction Stage" icon={Clock} subtitle="Current progress">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  'Agreement Stage', 'Pre-Registration', 
                  'Already Purchased', 'Suspicion/Dispute Exists'
                ].map((stage) => (
                  <button
                    key={stage}
      ...
      case 3:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Documents & <span className="text-luxury-gold">Depth.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Define available inputs and verification intensity.
              </p>
            </div>

            <SectionWrapper title="Document Availability" icon={ClipboardCheck} subtitle="Multi-select inputs">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Sale Deed', 'Title Chain', 'Encumbrance (EC)', 
                  'Khata', 'Approved Plan', 'Tax Receipts', 
                  'Conversion Cert', 'RTC', 'OC/CC', 
                  'Identity Proofs', 'Partial Docs'
                ].map((doc) => (
                  <button
                    key={doc}
                    onClick={() => {
            ...
            <SectionWrapper title="Verification Depth" icon={FileSearch} subtitle="Intensity of check">
              <div className="grid grid-cols-1 gap-3">
                {[
                  'Basic Authenticity Check', 
                  'Cross-Verification with Records', 
                  'Background Verification (Owner/Builder)', 
                  'Full Verification with Risk Flagging'
                ].map((depth) => (
                  <button
                    key={depth}
      ...
      case 4:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Concerns & <span className="text-luxury-gold">Site Visit.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Highlight red flags and physical verification needs.
              </p>
            </div>

            <SectionWrapper title="Specific Concerns" icon={ShieldAlert} subtitle="Multi-select red flags">
              <div className="grid grid-cols-2 gap-2">
                {[
                  'Fake Documents Risk', 'Ownership Mismatch', 'Hidden Encumbrance', 
                  'Khata Authenticity', 'Conversion Validity', 'Approval Legitimacy', 
                  'Litigation Risk', 'Boundary Mismatch', 'Builder Credibility', 
                  'Identity Fraud Risk'
                ].map((concern) => (
                  <button
                    key={concern}
                    onClick={() => {
                      const newConcerns = state.specificConcerns.includes(concern)
                        ? state.specificConcerns.filter(c => c !== concern)
                        : [...state.specificConcerns, concern];
                      setState({ ...state, specificConcerns: newConcerns });
                    }}
                    className={`p-4 rounded-2xl text-[11px] font-bold border transition-all relative flex items-center justify-center text-center ${
                      state.specificConcerns.includes(concern)
                        ? 'bg-luxury-gold/10 border-luxury-gold text-luxury-gold shadow-lg shadow-luxury-gold/5'
                        : 'bg-[var(--bg)] border-[var(--line)] text-[var(--muted)] hover:bg-[var(--paper)]'
                    }`}
                  >
            ...
            <SectionWrapper title="Physical Verification" icon={Map} subtitle="On-ground check">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Is Site Visit Required?</p>
                <SegmentedControl 
                  options={['Yes', 'No']} 
                  selected={state.siteVerificationRequired} 
                  onChange={(val) => setState({ ...state, siteVerificationRequired: val })} 
                />
              </div>
            </SectionWrapper>
          </motion.div>
      ...
      case 5:
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              <h2 className="font-display text-3xl font-black text-[var(--ink)] leading-tight">
                Output & <span className="text-luxury-gold">Timeline.</span>
              </h2>
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Specify deliverable format and urgency.
              </p>
            </div>

            <SectionWrapper title="Output Expectations" icon={FileText} subtitle="Deliverable format">
              <div className="space-y-4">
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1">Preferred Report</p>
                <SegmentedControl 
                  options={['Basic Report', 'Detailed Report', 'Risk Summary', 'Validation Proof', 'Go/No-Go Rec']} 
                  selected={state.outputExpectation} 
                  onChange={(val) => setState({ ...state, outputExpectation: val })} 
                />
                <p className="text-[10px] font-bold text-[var(--muted)] uppercase tracking-wider ml-1 mt-4">Consultation Required?</p>
                <SegmentedControl 
                  options={['Yes', 'No']} 
            ...
            <SectionWrapper title="Additional Notes" icon={Info} subtitle="Optional context">
              <textarea 
                value={state.specificNote}
                onChange={(e) => setState({ ...state, specificNote: e.target.value })}
                placeholder="Any other specific details or notes for the verification expert..."
        ...
        return (
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="grid grid-cols-3 gap-4 items-stretch">
            <div className="col-span-3 space-y-2">
              ...
              <p className="text-[var(--muted)] font-semibold text-xs leading-relaxed">
                Choose your verification expert or let us suggest the best fit.
              </p>
            </div>
              ...
              {state.selectionMode === 'manual' && (
                ...
                >
                  <div className="flex items-center justify-between px-2">
                    <h3 className="text-[11px] font-black text-[var(--ink)] uppercase tracking-wider">Verification Experts</h3>
                ...
                >
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-[var(--muted)] text-xs font-medium leading-relaxed">
                      Our system will analyze your verification purpose, scope, and jurisdiction to route your RFQ to the most qualified legal verification expert.
                    </p>
                  </div>
          ...
          >
            <div className="text-center space-y-2">
              <div className="w-20 h-20 bg-luxury-gold/10 rounded-full flex items-center justify-center mx-auto mb-4">
                <Fingerprint className="w-10 h-10 text-luxury-gold" />
              </div>
              <h2 className="text-2xl font-black text-[var(--ink)]">Review & Submit</h2>
              <p className="text-[var(--muted)] text-xs font-semibold">Verify your legal verification requirements.</p>
            </div>
            ...
            <div className="space-y-6">
              <ReviewSection 
                ...
                items={[
                  ...
                  { label: 'Scope', value: state.verificationScope }
                ]}
              />
              ...
              <ReviewSection 
                title="Concerns & Site Visit" 
                icon={ShieldAlert} 
                items={[
                  { label: 'Concerns', value: state.specificConcerns },
                  { label: 'Site Visit', value: state.siteVerificationRequired }
                ]}
              />

              <ReviewSection 
                title="Output & Timeline" 
                icon={Calendar} 
                items={[
                  { label: 'Output', value: state.outputExpectation },
          ...
          >
            ...
            <div className="space-y-3">
              <h2 className="text-3xl font-black text-[var(--ink)] leading-tight">
                Verification RFQ <span className="text-luxury-gold">Submitted!</span>
              </h2>
              <p className="text-[var(--muted)] text-sm font-semibold max-w-xs mx-auto">
                Thank you, {state.userName}. Your legal verification request has been sent to the selected experts.
              </p>
            </div>
    ...
    <div className="fixed inset-0 bg-premium-cream z-[60] flex flex-col overflow-hidden">
      ...
      <div className="bg-[var(--paper)]/80 backdrop-blur-xl border-b border-[var(--line)] px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex flex-col items-center">
          <h1 className="text-sm font-bold text-[var(--ink)] tracking-tight">Legal Verification</h1>
